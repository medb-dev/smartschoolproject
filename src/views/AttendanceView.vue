  ...
  <div>
    ...
    <div class="two-col">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div class="live-badge">
            ...
            <span class="font-mono" style="font-size:11px;color:#888">ESP32 connecté</span>
          </div>
        </div>
        <AttendanceLiveList :items="attendanceStore.todayAttendances.length
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="card">
          <div class="card-header">
            <span class="card-title">Marquer manuellement</span>
          </div>
          <div class="manual-form">
            <input v-model="manualSearch" type="text" placeholder="Rechercher un étudiant..." class="search-input" />
...
const activeRooms = ref([
  ...
  { id:2, subject:'Physique-Chimie', name:'Salle B', teacher:'Prof. Ouali', time:'14h30–16h30', present:18, pending:4, color:'#EF9F27' },
...
function markManual(status) {
  ...
  // TODO: integrate with attendanceStore.markAttendance()
  manualSearch.value = ''
}

let unsubscribe = null
onMounted(() => {
  // TODO: replace with real courseId/sessionId from active session
  // unsubscribe = attendanceStore.listenSession(courseId, sessionId)
})
onUnmounted(() => attendanceStore.stopListening())
</script>

<style scoped>
.tabs {
  display: flex; gap: 2px;
  background: #f0f0ee; padding: 3px;
...
.stat-red   { background: #FCEBEB; }
.stat-blue  { background: #E6F1FB; }
.stat-mini-val {
  font-size: 22px; font-weight: 600;
  font-family: 'IBM Plex Mono', monospace; line-height: 1;
}
.stat-green .stat-mini-val { color: #085041; }
.stat-amber .stat-mini-val { color: #633806; }
.stat-red   .stat-mini-val { color: #791F1F; }
.stat-blue  .stat-mini-val { color: #0C447C; }
...
.room-card {
  padding: 12px; border: 0.5px solid rgba(0,0,0,0.1);
...
.status-btn {
  ...
  border: 0.5px solid; font-size: 12px; font-weight: 500;
